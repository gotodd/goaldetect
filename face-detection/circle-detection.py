...
def put_text(image,text,pos):
    ...
    thickness = 1
    image = cv2.putText(image, text, pos, font, fontScale, color, thickness, cv2.LINE_AA)

def ball_recog_method1(filtered_image,hsv_img):
    h, s, result = cv2.split(hsv_img)
    #result=cv2.medianBlur(result,5)

    circles=cv2.HoughCircles(result, cv2.HOUGH_GRADIENT, 1, 40, param1=150,
...
def ball_recog_method2(raw_image,hsv_img):
    ...
    #cv2.RETR_TREE
    contours, h = cv2.findContours(edge_detected_image, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
    ...
    for contour in contours:
        ...
        if ((len(approx) > 8) & (len(approx) < 30) & (area > 400)):
            if circularity>=0.4:
                M = cv2.moments(contour)
                cx = int(M['m10']/M['m00'])
                cy = int(M['m01']/M['m00'])
                put_text(raw_image,str(len(approx))+", "+str(area)+", "+str(perimeter),(cx,cy))
                count+=1
                print(count,"Points ",len(approx),"Area ",area," X,Y ",cx,cy)
...
raw_image = cv2.imread('/Users/tzhang/Downloads/frc-ball.jpg') #frc-ball.jpg 
filtered_image = cv2.bilateralFilter(raw_image, 12, 175, 175)
